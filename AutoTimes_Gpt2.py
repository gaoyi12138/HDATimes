...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        if configs.mlp_hidden_layers == 0:
            ...
            self.decoder = MLP(self.hidden_dim_of_gpt2, self.token_len,
                            configs.mlp_hidden_dim, configs.mlp_hidden_layers,
                            configs.dropout, configs.mlp_activation)
        self.proj_x_mark_enc = nn.Linear(8, self.hidden_dim_of_gpt2)  #wind:12  exchange_rate 9  illness 8
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        if self.mix:
            ...
            # 在模型初始化时定义一个线性层  wind数据集
            x_mark_enc = self.proj_x_mark_enc(x_mark_enc)  # 映射后的形状是 [256, 7, 768]
            times_embeds = times_embeds + self.add_scale * x_mark_enc
        # outputs: [bs * n_vars x token_num x hidden_dim_of_gpt2]
        #times_embeds shape: torch.Size([256, 7, 768]), dtype: torch.float32, device: cuda:0

        outputs = self.gpt2(inputs_embeds=times_embeds).last_hidden_state  #times_embeds.shape:[256,7,768]
