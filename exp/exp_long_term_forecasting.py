...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.use_multi_gpu:
            self.device = torch.cuda.set_device(f"{self.args.local_rank}")
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                for j in range(inference_steps):
                    if len(pred_y) != 0:
                        ...
                        #print(f"pred_y[-1] shape: {pred_y[-1].shape}")
                        a,b,c=pred_y[-1].size()

                        batch_x = batch_x[:, self.args.token_len:, :].expand(-1, -1, c)  # 变为 [256, 2]，移除维度 1
                        batch_x = torch.cat([batch_x, pred_y[-1]], dim=1)
