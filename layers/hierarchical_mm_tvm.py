...
class GraphMM(torch.autograd.Function):
    ...
    def _graph_mm(t1: torch.Tensor, t2: torch.Tensor, q_k_mask: torch.Tensor, k_q_mask: torch.Tensor,
        ...
        b = t1.shape[0]  # batch size 256
        n = t1.shape[1]  # sequence length 768
        h = t1.shape[2]  # number of heads 4
        m = t2.shape[3]  # hidden dimension 128
        #max_attn = q_k_mask.size(1)
        max_attn = q_k_mask.shape[1]  # 使用 .shape 属性,max_attn=64
        if is_t1_diagonaled:
            ...
            assert t1.shape[3] == m
            r = t1.new_empty(b, n, h, int(max_attn))  # allocate spase for the result tensor
        ...
        #print("k_q_mask.type:",type(k_q_mask)) ,k_q_mask.shape: <class 'tvm.te.tensor.Tensor'>
        print("k_q_mask.type:", type(k_q_mask))  #k_q_mask.type: <class 'torch.Tensor'>
        print("q_k_mask.type:", type(q_k_mask))  #k_q_mask.type: <class 'torch.Tensor'>
        print(q_k_mask.dtype)#torch.int32
        print(k_q_mask.dtype) #torch.int32
        print(q_k_mask.shape)  # torch.Size([768, 64])
        print(k_q_mask.shape)# torch.Size([768, 64])
