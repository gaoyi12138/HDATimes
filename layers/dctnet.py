...
class dct_channel_block(nn.Module):
    ...
    def forward(self, x):
       ...
       # stack_dct = torch.tensor(stack_dct)
        ...
        '''
        #print("stack_dct shape:", stack_dct.shape)  #(256,7,96)
        # 重新调整形状以适应线性层
        stack_dct = stack_dct.view(b * c, l)  # 变为 (B * C, L)  (1792,96)
        lr_weight = self.dct_norm(stack_dct)  #归一化操作 形状依旧为（1792，96）
        lr_weight = self.fc(lr_weight)  # 变为 (1792, 768)
        # 然后将其形状调整为 (B, C, 768)
        lr_weight = lr_weight.view(b, c, 96)  # 变为 (256, 7, 768)
