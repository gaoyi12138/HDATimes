...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Ensure the rest of the model parameters are in float32
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        x_enc = x_enc.to(self.device).to(torch.float16)
        x_mark_enc = x_mark_enc.to(self.device).to(torch.float16)
        x_dec = x_dec.to(self.device).to(torch.float16) if x_dec is not None else None
        x_mark_dec = x_mark_dec.to(self.device).to(torch.float16)
        ...
        fold_out = fold_out.to(torch.float16)
        ...
        times_embeds = times_embeds.to(torch.float16)
